...
function AssignRoles() {
    ...
    const [STRplace, setSTRplace] = useState("");
    ...
    const loadBlockchaindata = async () => {
        ...
        if (networkData) {
            ...
            for (let i = 0; i < strCtr; i++) {
                try {
        // Assuming STR is correctly implemented in the smart contract
                str[i] = await supplychain.methods.STR(i + 1).call();
                } catch (error) {
                console.error(`Error fetching STR at index ${i}:`, error);
        // Handle or ignore the error based on your needs
            }
            }   

// Safely update the state after the loop is complete
setSTR(str);
    ...
    const handlerChangeAddressSTR = (event) => {
        setSTRaddress(event.target.value);
    }

    const handlerChangePlaceSTR = (event) => {
        setSTRplace(event.target.value);
    }

    const handlerChangeNameSTR = (event) => {
    ...
    const handlerSubmitSTR = async (event) => {
        event.preventDefault();
        try {
            var receipt = await SupplyChain.methods.addSTR(STRaddress, STRname, STRplace).send({ from: currentaccount });
            if (receipt) {
                loadBlockchaindata();
            }
        } catch (err) {
    ...
    return (
        <div className="container mt-4">
            ...
            <div className="row">
                ...
                <div className="col-md-12 mb-3">
                    <h4>STR centres</h4>
                    <form onSubmit={handlerSubmitSTR} className="form-inline mb-3">
                        <input className="form-control mr-2" type="text" onChange={handlerChangeAddressSTR} placeholder="Ethereum Address" required />
                        <input className="form-control mr-2" type="text" onChange={handlerChangeNameSTR} placeholder="Retailer Name" required />
                        <input className="form-control mr-2" type="text" onChange={handlerChangePlaceSTR} placeholder="Based In" required />
                        <button className="btn btn-primary" type="submit">Register</button>
                    </form>
                    {renderTable(STR, "STR")}
                </div>
